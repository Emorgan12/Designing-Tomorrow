...
Once the required packages are installed, you will need to ensure that an Arduino is connected via USB to the Pi, with a light bulb connected to it with the short pin in GND and the long pin in 13. For what this will be used for the code can be found [here](arduino_light/rfid_flash.py). Ensure the Arduino port is correct on line 7 (e.g. `/dev/ttyACM0`) — run `ls /dev/ttyACM*` to confirm.
